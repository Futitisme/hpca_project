...
The implementation follows the same optimization algorithm, numerical precision, initialization strategy, and convergence behavior as the CPU baseline, it does GPU parallelism via CUDA and cuBLAS.
...
- NVIDIA CUDA Toolkit
- cuBLAS
- zlib (for `.npy` loading using `cnpy`)
